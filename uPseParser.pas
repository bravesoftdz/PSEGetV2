...
type
  TSectorStatus = (secUp, secDown, secUnch);
  ...
  public
    ...
    property Status: TSectorStatus read FStatus write FStatus;
  ...
  public
    ...
    property TradeDate: TDateTime read FTradeDate;
...
function GetSectorStatus(Value: String): TSectorStatus;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if (m > 10) and (Trim(QuoteDocument[m]) <> '') then
        begin
    ...
    begin
      ...
      begin
        ...
        if (SectorCollection.Code = '^ALLSHARES') or
           (SectorCollection.Code = '^PHISIX')  then
        begin
          SectorCollection.Change := CleanupFloatValue(StrArray[j]);
          SectorCollection.Status := GetSectorStatus(StrArray[j - 1]);
          SectorCollection.Close := CleanupFloatValue(StrArray[j - 2]);

          if SectorCollection.Code = '^PHISIX' then
          begin
        ...
        begin
          ...
          SectorCollection.Status := GetSectorStatus(StrArray[j - 3]);
...
var
  ...
  procedure WriteStockLine(APersistent: TPersistent);
  var
    S, S1: String;
    k: Integer;
  begin
